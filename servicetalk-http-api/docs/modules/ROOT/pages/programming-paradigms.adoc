...
This limit covers aggregation performed by ServiceTalk. Application or framework code that consumes the streaming
body and buffers it itself is not covered -- for example the JAX-RS entity readers of the
xref:{page-version}@servicetalk-http-router-jersey::index.adoc#payload-size-limits[Jersey router]. To bound request
payloads regardless of who performs the buffering, add `PayloadSizeLimitingHttpServiceFilter` (from
`servicetalk-http-utils`) to the server.
